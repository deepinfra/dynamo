# LTX-2.3 Video Pipeline Runbook

> **LTX-2.3 variant (deepinfra).** Copied from the LTX-2 RUNBOOK; the body below
> still uses LTX-2 paths/names in places. For the LTX-2.3 image, substitute:
> - dir `examples/diffusers/ltx2/` → `examples/diffusers/ltx23/`
> - container path `/opt/app/ltx2/shapes.json` → `/opt/app/ltx23/shapes.json`
> - worker dispatch: set `VIDEO_MODEL_FAMILY=ltx23` (top-level shim env-dispatch)
> - image tag `…-ltx2-<hash>` → `…-ltx23-<hash>`; model `FastVideo/LTX-2.3-Distilled-Diffusers`
> - shape menu: 2 shapes (1080p + 720p landscape, 5s) — see `ltx23/shapes.json`
> - FastVideo SHA pinned to `633d3935` (NVFP4 renamed → `NVFP4Config`); NVFP4 compile
>   defaults to `mode="default"` (set `LTX23_FP4_MAX_AUTOTUNE=1` to A/B max-autotune)
> - eager warm-on-boot is DEFAULT (`LTX23_EAGER_WARM=1`); startup-probe budget raised to 30 min
>
> TODO: full path sweep of this doc for the ltx23 image.
