...
"""
FastWan2.2-TI2V-5B worker entry point for Dynamo (non-streaming).
...
This module is the FastWan2.2-TI2V-5B-specific entry. The top-level
...
DEFAULT_MODEL = "FastVideo/FastWan2.2-TI2V-5B-FullAttn-Diffusers"
# FLASH_ATTN: bf16 flash-attn2, the same backend the H100 benchmarks ran.
# TORCH_SDPA is the slow generic fallback.
DEFAULT_ATTENTION_BACKEND = "FLASH_ATTN"
DEFAULT_MODEL_LABEL = "fastwan22-ti2v-5b"

# Where the FastWan shapes JSON lives in the production image. Production
...
PRODUCTION_SHAPES_JSON_PATH = "/opt/app/fastwan22_5b/shapes.json"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Accepted for shared CLI compat; the recipe has no gated optimizations",
    )
    parser.add_argument(
        "--attention-backend",
        choices=choices,
...
async def _main(args: argparse.Namespace) -> None:
    ...
    # Eager warm-on-boot is the DEFAULT. The v1 menu is 2 shapes with no torch.compile,
    # so warming every shape's pool subprocess at boot is cheap. It
...
def main_cli() -> None:
    """
    FastWan2.2-TI2V-5B worker entry. Called by the top-level ``worker.py`` shim
...
if __name__ == "__main__":
    # If invoked directly (e.g. `python3 -m fastwan22_5b.worker`), guard the
    ...
    if "--pool-worker" in sys.argv:
        raise SystemExit(
            "fastwan22_5b.worker invoked directly with --pool-worker; "
