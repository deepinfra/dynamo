...
DEFAULT_MODEL_LABEL = "ltx2-distilled"

# Where the LTX-2 shapes JSON lives in the production image. Production
...
PRODUCTION_SHAPES_JSON_PATH = "/opt/app/ltx2/shapes.json"
...
async def _main(args: argparse.Namespace) -> None:
    ...
    # cache, it pays full first-call cost per shape -- ~25 minutes for
    # our 10-shape menu. For low-volume soft-launch traffic that's worse
    # than accepting that the first customer per shape per pod pays the
    # cold cost. Re-enable when traffic patterns make the trade-off
    # favor uniform latency at the cost of long pod boots.
