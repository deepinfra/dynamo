...
impl Service {
    ...
    async fn start_with(
        ...
        // Mirror what the discovery watcher builds for a real model: one tokenizer, a
        // chat preprocessor when the card has a template, and a no-op-formatter
        // preprocessor for the completions side. /tokenize reads these, so registering
        // only the card would not exercise the same objects production does.
...
async fn tokenize_completion_matches_dynamo_not_vllm_defaults() {
    ...
    // this is where the endpoint deliberately departs from vLLM's default of true.
...
async fn tokenize_completion_rejects_add_special_tokens() {
    ...
    // Accepted by the schema for vLLM compatibility; refused rather than answered with a
    // count this deployment would never produce.
    let resp = svc
        .post(
            "/tokenize",
...
async fn tokenize_chat_rejects_multimodal_content() {
    ...
    // A single `<|image|>` placeholder in the rendered template is nowhere near the
    // token count the image actually expands to, so refuse rather than under-report.
    let resp = svc
        .post(
            "/tokenize",
            json!({
...
async fn tokenize_requires_the_models_pipeline_preprocessor() {
    // A card alone is not enough: the endpoints read the preprocessor the generate
    // pipeline was built with. Registering the card without one must fail loudly rather
    // than fall back to a second tokenizer built here — that fallback is exactly the
    // drift this plumbing exists to prevent.
    let (listener, port) = bind_random_port().await;
    let service = HttpService::builder()
        .port(port)
        .host("127.0.0.1")
