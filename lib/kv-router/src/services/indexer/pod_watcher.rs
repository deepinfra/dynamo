...
//!
//! Each pod is registered under `routing_group = <its engine_hash label>`, so
//! engine generations that hash KV blocks differently never share a tree
//! during a migration; `/query` fans out over a model's routing groups.
//!
//! This module is the *only* place that imports the Kubernetes client. It
//! depends on [`WorkerRegistry`] and calls nothing but
//! [`WorkerRegistry::register_with_extras`] / [`WorkerRegistry::deregister`],
//! so the indexer core never learns about Kubernetes.
//!
...
use super::discovery::KubeDiscoveryConfig;
...
/// blocks differently, so each value gets its own tree via `routing_group`.
...
impl DiscoveredPod {
    /// The routing group this pod's tree lives under: its `engine_hash`, so
    /// hash-incompatible engine generations never share a tree during
    /// migrations. Falls back to the configured group for unlabeled pods.
    fn routing_group(&self, fallback: &str) -> String {
        self.engine_hash
            .clone()
...
async fn reconcile(
    ...
    // routing group, and register() rejects a worker that switches keys in place).
    let vanished: Vec<String> = subscribed
        .iter()
        .filter(|(name, entry)| {
            desired
                .get(*name)
                .is_none_or(|pod| pod.routing_group(&config.routing_group) != entry.routing_group)
        })
    ...
    for name in vanished {
        ...
        match registry
            .deregister(entry.instance_id, &config.model_name, &entry.routing_group)
            .await
        {
            Ok(()) => tracing::info!(
                pod = %name,
                ...
                routing_group = %entry.routing_group,
    ...
    // Subscribe to pods that are newly Ready (or regrouped above).
    for (name, pod) in &desired {
        ...
        let ip = &pod.ip;
        let routing_group = pod.routing_group(&config.routing_group);
        ...
        match failed {
            ...
            Some((dp_rank, error)) => {
                ...
                if let Err(error) = registry
                    .deregister(instance_id, &config.model_name, &routing_group)
                    .await
                {
                    tracing::debug!(
                        pod = %name,
...
mod tests {
    ...
    #[test]
    fn engine_hash_label_becomes_routing_group() {
        ...
        assert_eq!(discovered.routing_group("default"), "975d72d83374a79b");
    }

    #[test]
    fn missing_engine_hash_falls_back_to_configured_routing_group() {
        let p = pod_with_labels(
            "engine-abc",
            Some("10.0.0.1"),
            Some("True"),
        ...
        assert_eq!(discovered.routing_group("default"), "default");
    }
