...
/// `--keep-evictions` settings.
#[derive(Debug, Clone, Copy)]
pub struct KeepEvictionsConfig {
    /// Minimum age a parked eviction must reach before the sweep replays it.
    pub retention_s: u64,
    /// Fraction of the memory limit, in (0, 1], above which the sweep runs.
    pub memory_threshold: f64,
}

impl KeepEvictionsConfig {
    pub fn validate(&self) -> anyhow::Result<()> {
        anyhow::ensure!(
            self.memory_threshold > 0.0 && self.memory_threshold <= 1.0,
...
pub fn spawn_cleanup_loop(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for (worker_id, dp_rank, pending_evictions, indexer) in registry.listener_records() {
                ...
                for (tier, hashes) in doomed {
                    ...
                    for chunk in hashes.chunks(REMOVE_CHUNK) {
                        ...
                        if let Err(error) = indexer.apply_event_routed(event).await {
                            tracing::warn!(
                                worker_id,
                                dp_rank,
                                %error,
                                "keep-evictions sweep: replayed eviction was rejected"
                            );
                        }
                    }
                }
...
mod tests {
    ...
    async fn drained_evictions_replay_into_the_tree() {
        ...
        use crate::services::indexer::backend::{create_indexer, test_util::store_event};
