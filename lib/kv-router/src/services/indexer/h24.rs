...
mod tests {
    ...
    /// Every routing group of a model shares ONE h24 map, and deregistration
    /// with a pod's real routing group still finds it.
    #[tokio::test]
    async fn registry_collapses_routing_groups_into_one_h24_indexer() {
        use crate::services::indexer::registry::RegistryOptions;
        ...
        for (instance, group) in [(1u64, "regime-a"), (2u64, "regime-b")] {
            registry
                .register(
                    instance,
                    format!("tcp://127.0.0.1:1576{instance}"),
                    0,
                    "m".to_string(),
                    group.to_string(),
                    4,
        ...
        let trees = registry.indexers_for_model("m");
        assert_eq!(trees.len(), 1, "h24 keeps one tree per model");
        assert_eq!(trees[0].0.routing_group, "h24");
        assert!(matches!(trees[0].1, Indexer::H24(_)));

        registry.deregister(1, "m", "regime-a").await.unwrap();
        registry.deregister(2, "m", "regime-b").await.unwrap();
